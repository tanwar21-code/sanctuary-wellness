...
export default function LoginPage() {
  const { login, dbUser, loading } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (!loading && dbUser) {
      if (dbUser.role === 'counsellor') {
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(135deg, #ede9fe 0%, #f0f9ff 50%, #ecfdf5 100%)',
      position: 'relative',
      overflow: 'hidden',
    }}>
      {/* Floating blobs */}
      ...
      <div className="glass-card animate-slideUp" style={{
        ...
        <button
          ...
          style={{
            width: '100%', justifyContent: 'center', padding: '14px 28px',
